...
merged_data <- wf_raions_summarized %>%
  inner_join(summarized_dataset2, by = c("raion_name")) %>% 
...
scatter_plot <- ggplot(wf_idp_raion2, aes(x = wf_intensity, y = net_pop_change, text = raion_name)) +
  geom_point() +
...
### Correlation Chart - War-fire events and IDP by week

```{r}
weekly_correlation <- merged_data2 %>%
  group_by(week_number, week) %>%
  summarize(correlation = cor(wf_count, net_pop_change, use = "complete.obs"),
            net_pop_change = sum(net_pop_change),
            wf_count = sum(wf_count))
...
ggplot(weekly_correlation, aes(x = week, y = correlation)) +
  geom_line() +
  geom_point() +
  labs(title = "War-Fire Events and Net Population Change Correlation by Week",
       x = "Week",
       y = "Correlation") +
  scale_y_reverse() +
  #scale_x_continuous(breaks = seq(0, max(weekly_correlation$week_number), by = 5)) +
  scale_x_date(breaks = seq(min(weekly_correlation$week), max(weekly_correlation$week), by = "5 weeks"),
               labels = date_format("%Y-%m-%d")) +
  theme_minimal()

# save it
ggsave(filename = "output/figures/weekly_correlation.png", plot = scatter_plot, dpi = 300)
```

```{r}
# create the static scatterplot for specific weeks
selected_week_number = 5

# plot the scatterplot
selected_week_scatter_plot <- ggplot(merged_data2 %>% filter(week_number == selected_week_number), aes(x = wf_count, y = net_pop_change, text = raion_name)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Scatterplot between War-Fire Intensity and Net Population Change",
       x = "War Fire Events Count",
       y = "Net Population Change")

# make the scatterplot interactive
interactive_scatter_selected_week <- ggplotly(selected_week_scatter_plot)

# display the interactive scatterplot
interactive_scatter_selected_week
```
...
# create the static scatterplot
scatter_plot <- ggplot(merged_data2 %>% filter(week_number == selected_week_number), aes(x = wf_intensity, y = net_pop_change)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = paste("War-Fire Intensity and Net Population Change - Week", selected_week_number),
       x = "War Fire Intensity",
       y = "Net Population Change")
