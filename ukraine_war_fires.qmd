...
```{r}
old_raion <- read.csv('data/displacement_data/raions.csv')
```
...
summarized_dataset <- idp_raion %>%
  ...
  summarize(
    net_refugee_outflow = sum(net_refugee_outflow, na.rm = TRUE),
...
merged_data <- raion_weekly %>%
  inner_join(summarized_dataset, by = c("raion_name", "week_number")) %>%
...
calculate_correlation <- function(data) {
  cor(data$wf_count, data$net_pop_change, use = "complete.obs")
}

wf_idp_raion <- merged_data %>%
  group_by(raion_name) %>%
  summarize(total_net_pop_change = sum(net_pop_change, na.rm = TRUE),
            sum_wf_count = sum(wf_count, na.rm = TRUE),
...
wf_idp_raion <- wf_idp_raion %>%
  mutate(wf_intensity = (sum_wf_count - min(sum_wf_count, na.rm = TRUE)) / 
                                   (max(sum_wf_count, na.rm = TRUE) - min(sum_wf_count, na.rm = TRUE))) %>%
  rename(
    net_pop_change = total_net_pop_change,
    wf_count = sum_wf_count
  )
```

```{r}
cat("The correlation at raion level combining all weeks movements and counts is:", cor(wf_idp_raion$total_net_pop_change, wf_idp_raion$sum_wf_count, use = "complete.obs"), "\n")
```
...
```{r}
# Replace theme_tufte2 with theme_minimal for this example
wf_idp_raion %>% 
  ...
  # Add median migration rate by population density
